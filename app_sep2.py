...
def seird(
        s: float, e: float, i: float, r: float, d: float, beta: float, gamma: float, alpha: float, n: float,
        fatal: float
) -> Tuple[float, float, float, float]:
    """The SIR model, one time step."""
    s_n = (-beta * s * i) + s
...
def seijcrd(
        s: float, e: float, i: float, j: float, c: float, r: float, d: float, beta: float, gamma: float, alpha: float,
        n: float, fatal_hosp: float, hosp_rate: float, icu_rate: float, icu_days: float, crit_lag: float,
        death_days: float
) -> Tuple[float, float, float, float]:
    """The SIR model, one time step."""
    s_n = (-beta * s * (i + j + c)) + s
...
def derivdecay(y, t, N, beta, gamma1, gamma2, alpha, p, hosp, q, l, n_days, decay1, decay2, decay3, decay4, decay5,
               decay6, decay7, decay8, decay9, decay10, decay11, decay12,  # Change Here for time-point
               start_day, int1_delta, int2_delta, int3_delta, int4_delta, int5_delta,
               fatal_hosp):  # Change here time-point
...
def sim_seaijrd_decay_ode(
        s, e, a, i, j, r, d, beta, gamma1, gamma2, alpha, n_days, decay1, decay2, decay3, decay4, decay5, decay6,
        decay7, decay8, decay9, decay10, decay11, decay12,  # Change Here for time-point
        start_day, int1_delta, int2_delta, int3_delta, int4_delta, int5_delta, fatal_hosp, p, hosp, q,
        l):  # Change Here for time-point
    n = s + e + a + i + j + r + d
    rh = 0
    y0 = s, e, a, i, j, r, d, rh

    t = np.arange(0, n_days, step=1)
    ret = odeint(derivdecay, y0, t, args=(
    n, beta, gamma1, gamma2, alpha, p, hosp, q, l, n_days, decay1, decay2, decay3, decay4, decay5, decay6, decay7,
    decay8, decay9, decay10, decay11, decay12,  # Change Here for time-point
    start_day, int1_delta, int2_delta, int3_delta, int4_delta, int5_delta, fatal_hosp))  # Change Here for time-point
...
def betanew2(t, beta, x, p_m1, pm_2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11,
             p_m12):  # Change Here for time-point
    beta_decay = 0.0
    ...
    elif int9_delta < t <= int10_delta:
        beta_decay = beta * (1 - decay10) * (1 - (x * p_m10)) ** 2
    elif int10_delta < t <= int11_delta:
        beta_decay = beta * (1 - decay11) * (1 - (x * p_m11)) ** 2
    elif int11_delta < t < n_days:
        beta_decay = beta * (1 - decay12) * (1 - (x * p_m12)) ** 2
    return beta_decay


# Adding period specific changes to tranmission or percentage of population
# Updated 11/24/21
# This is for the new strain and fraction of the population with variants
def betanew3(t, beta, x, p_m1, pm_2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11, p_m12,
             # Change Here for time-point
             new_strain6, new_strain7, new_strain8, new_strain9, new_strain10, new_strain11, new_strain12,
             # Change Here for time-point
             fracNS6, fracNS7, fracNS8, fracNS9, fracNS10, fracNS11, fracNS12):
    beta_decay = 0.0
    if start_day <= t <= int1_delta:
        beta_decay = beta * (1 - decay1) * (1 - (x * p_m1)) ** 2
    elif int1_delta < t <= int2_delta:
        ...
        beta_decay = ((1 - fracNS6) * beta * (1 - decay6) * (1 - (x * p_m6)) ** 2) + (
                    fracNS6 * (1 + new_strain6) * beta * (1 - decay6) * (1 - (x * p_m6)) ** 2)
    elif int6_delta < t <= int7_delta:
        beta_decay = ((1 - fracNS7) * beta * (1 - decay7) * (1 - (x * p_m7)) ** 2) + (
                    fracNS7 * (1 + new_strain7) * beta * (1 - decay7) * (1 - (x * p_m7)) ** 2)
    elif int7_delta < t <= int8_delta:
        beta_decay = ((1 - fracNS8) * beta * (1 - decay8) * (1 - (x * p_m8)) ** 2) + (
                    fracNS8 * (1 + new_strain8) * beta * (1 - decay8) * (1 - (x * p_m8)) ** 2)
    elif int8_delta < t <= int9_delta:
        beta_decay = ((1 - fracNS9) * beta * (1 - decay9) * (1 - (x * p_m9)) ** 2) + (
                    fracNS9 * (1 + new_strain9) * beta * (1 - decay9) * (1 - (x * p_m9)) ** 2)
    elif int9_delta < t <= int10_delta:
        beta_decay = ((1 - fracNS10) * beta * (1 - decay10) * (1 - (x * p_m10)) ** 2) + (
                    fracNS10 * (1 + new_strain10) * beta * (1 - decay10) * (1 - (x * p_m10)) ** 2)
    elif int10_delta < t <= int11_delta:
        beta_decay = ((1 - fracNS11) * beta * (1 - decay11) * (1 - (x * p_m11)) ** 2) + (
...
# Modified 11/24/21 # Addind additional timepoint - now at 11
# Vaccine part
def phinew2(t, phi5, phi6, phi7, phi8, phi9, phi10, phi11, phi12):
    phi_decay = 0.0
    if start_day <= t <= int1_delta:
        phi_decay = 0
...
def derivdecayP(y, t, beta, gamma1, gamma2, alpha, sym, hosp, q, l, n_days, decay1, decay2, decay3, decay4, decay5,
                ...
                int8_delta, int9_delta, int10_delta, int11_delta,  # Change Here for time-point
                fatal_hosp, x, p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11, p_m12,
                delta_p):  # Change Here for time-point
    S, E, P, A, I, J, R, D, counter = y
    N = S + E + P + A + I + J + R + D
    dSdt = - betanew2(t, beta, x, p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11, p_m12) * S * (
                q * I + l * J + P + A) / N  # Change Here for time-point
    dEdt = betanew2(t, beta, x, p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11, p_m12) * S * (
                q * I + l * J + P + A) / N - alpha * E  # Change Here for time-point
...
def derivdecayV(y, t, beta, gamma1, gamma2, alpha, sym, hosp, q, l, n_days, decay1, decay2, decay3, decay4, decay5,
                ...
                start_day, int1_delta, int2_delta, int3_delta, int4_delta, int5_delta, int6_delta, int7_delta,
                int8_delta, int9_delta, int10_delta, int11_delta,  # Change Here for time-point
                fatal_hosp, x, p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11, p_m12, delta_p,
                sigma, phi):
    ...
    N = S + E + P + A + I + J + R + D + V
    dSdt = - betanew2(t, beta, x, p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11, p_m12) * S * (
                q * I + l * J + P + A) / N - (phi * S)  # Change Here for time-point
    dVdt = (phi * S) - (sigma * betanew2(t, beta, x, p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11,
                                         p_m12) * V * (q * I + l * J + P + A) / N)  # Change Here for time-point
    dEdt = (betanew2(t, beta, x, p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11, p_m12) * S * (
                q * I + l * J + P + A) / N) + (
                       sigma * betanew2(t, beta, x, p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11,
                                        p_m12) * V * (q * I + l * J + P + A) / N) - alpha * E
    dPdt = alpha * E - delta_p * P
    dAdt = delta_p * P * (1 - sym) - gamma1 * A
    dIdt = sym * delta_p * P - gamma1 * I - hosp * I
    dJdt = hosp * I - gamma2 * J
...
def derivdecayVtime(y, t, beta, gamma1, gamma2, alpha, sym, hosp, q, l, n_days, decay1, decay2, decay3, decay4, decay5,
                    decay6, decay7, decay8, decay9, decay10, decay11, decay12,
                    start_day, int1_delta, int2_delta, int3_delta, int4_delta, int5_delta, int6_delta, int7_delta,
                    int8_delta, int9_delta, int10_delta, int11_delta,
                    fatal_hosp, x, p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11, p_m12, delta_p,
                    sigma, phi):
    # here sigma=scaling rate of how effective the vaccine is
    # phi=rate suscepitble individuals are vaccinated at each time step
    ...
    N = S + E + P + A + I + J + R + D + V
    dSdt = - betanew2(t, beta, x, p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11, p_m12) * S * (
                q * I + l * J + P + A) / N - (phinew2(t, phi5, phi6, phi7, phi8, phi9, phi10, phi11, phi12) * S)
    dVdt = (phinew2(t, phi5, phi6, phi7, phi8, phi9, phi10, phi11, phi12) * S) - (
                sigma * betanew2(t, beta, x, p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11,
                                 p_m12) * V * (q * I + l * J + P + A) / N)
    dEdt = (betanew2(t, beta, x, p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11, p_m12) * S * (
                q * I + l * J + P + A) / N) + (
                       sigma * betanew2(t, beta, x, p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11,
                                        p_m12) * V * (q * I + l * J + P + A) / N) - alpha * E
...
def derivdecayVNS(y, t, beta, gamma1, gamma2, alpha, sym, hosp, q, l, n_days, decay1, decay2, decay3, decay4, decay5,
                  ...
                  int8_delta, int9_delta, int10_delta, int11_delta,  # Change Here for time-point
                  fatal_hosp, x, p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11, p_m12, delta_p,
                  sigma,  # Change Here for time-point
                  phi5, phi6, phi7, phi8, phi9, phi10, phi11, phi12,  # Change Here for time-point
                  new_strain6, new_strain7, new_strain8, new_strain9, new_strain10, new_strain11, new_strain12,
                  # Change Here for time-point
                  fracNS6, fracNS7, fracNS8, fracNS9, fracNS10, fracNS11, fracNS12):  # Change Here for time-point
    # here sigma=scaling rate of how effective the vaccine is
    # phi=rate suscepitble individuals are vaccinated at each time step
    S, V, E, P, A, I, J, R, D, counter = y
    N = S + E + P + A + I + J + R + D + V
    dSdt = - betanew3(t, beta, x, p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11, p_m12,
                      new_strain6, new_strain7, new_strain8, new_strain9, new_strain10, new_strain11, new_strain12,
                      fracNS6, fracNS7, fracNS8, fracNS9, fracNS10, fracNS11, fracNS12) * S * (
                       q * I + l * J + P + A) / N - (
                       phinew2(t, phi5, phi6, phi7, phi8, phi9, phi10, phi11, phi12) * S)  # Change Here for time-point
    dVdt = (phinew2(t, phi5, phi6, phi7, phi8, phi9, phi10, phi11, phi12) * S) - (
                sigma * betanew3(t, beta, x, p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11, p_m12,
                                 new_strain6, new_strain7, new_strain8, new_strain9, new_strain10, new_strain11,
                                 new_strain12, fracNS6, fracNS7, fracNS8, fracNS9, fracNS10, fracNS11, fracNS12) * V * (
                            q * I + l * J + P + A) / N)  # Change Here for time-point
    dEdt = (betanew3(t, beta, x, p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11, p_m12, new_strain6,
                     new_strain7, new_strain8, new_strain9, new_strain10, new_strain11, new_strain12, fracNS6, fracNS7,
                     fracNS8, fracNS9, fracNS10, fracNS11, fracNS12) * S * (q * I + l * J + P + A) / N) + (
                       sigma * betanew3(t, beta, x, p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11,
                                        p_m12, new_strain6, new_strain7, new_strain8, new_strain9, new_strain10,
                                        new_strain11, new_strain12, fracNS6, fracNS7, fracNS8, fracNS9, fracNS10,
                                        fracNS11, fracNS12) * V * (q * I + l * J + P + A) / N) - alpha * E
    dPdt = alpha * E - delta_p * P
    dAdt = delta_p * P * (1 - sym) - gamma1 * A
    dIdt = sym * delta_p * P - gamma1 * I - hosp * I
    dJdt = hosp * I - gamma2 * J
...
def sim_sepaijrd_decay_ode(
        s, e, p, a, i, j, r, d, beta, gamma1, gamma2, alpha, n_days, decay1, decay2, decay3, decay4, decay5, decay6,
        decay7, decay8, decay9, decay10, decay11, decay12,
        start_day, int1_delta, int2_delta, int3_delta, int4_delta, int5_delta, int6_delta, int7_delta, int8_delta,
        int9_delta, int10_delta, int11_delta,
    ...
    ret = odeint(derivdecayP, y0, t, args=(beta, gamma1, gamma2, alpha, sym, hosp, q, l, n_days,
                                           ...
                                           decay10, decay11, decay12,  # Change Here for time-point
                                           start_day, int1_delta, int2_delta, int3_delta, int4_delta, int5_delta,
                                           int6_delta, int7_delta, int8_delta, int9_delta, int10_delta, int11_delta,
                                           fatal_hosp, x,  # Change Here for time-point
                                           p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11, p_m12,
                                           delta_p))  # Change Here for time-point
...
def sim_svepaijrd_decay_ode(
        s, v, e, p, a, i, j, r, d, beta, gamma1, gamma2, alpha, n_days, decay1, decay2, decay3, decay4, decay5, decay6,
        decay7, decay8, decay9, decay10, decay11, decay12,  # Change Here for time-point
        start_day, int1_delta, int2_delta, int3_delta, int4_delta, int5_delta, int6_delta, int7_delta, int8_delta,
        int9_delta, int10_delta, int11_delta,  # Change Here for time-point
        fatal_hosp, sym, hosp, q,
        l, x,
        p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11, p_m12, delta_p, sigma,
        phi):  # Change Here for time-point
    n = s + v + e + p + a + i + j + r + d
    rh = 0
    ...
    ret = odeint(derivdecayVtime, y0, t, args=(beta, gamma1, gamma2, alpha, sym, hosp, q, l, n_days,
                                               decay1, decay2, decay3, decay4, decay5, decay6, decay7, decay8, decay9,
                                               decay10, decay11, decay12,
                                               start_day, int1_delta, int2_delta, int3_delta, int4_delta, int5_delta,
                                               int6_delta, int7_delta, int8_delta, int9_delta, int10_delta, int11_delta,
                                               fatal_hosp, x,  # Change Here for time-point
                                               p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11,
                                               p_m12, delta_p, sigma, phi))  # Change Here for time-point
...
def sim_svepaijrdNS_decay_ode(
        ...
        fatal_hosp, sym, hosp, q,
        l, x,
        p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11, p_m12, delta_p, sigma, phi5, phi6, phi7,
        phi8, phi9, phi10, phi11, phi12, new_strain6, new_strain7, new_strain8, new_strain9, new_strain10, new_strain11,
        new_strain12, fracNS6, fracNS7, fracNS8, fracNS9, fracNS10, fracNS11, fracNS12):  # Change Here for time-point
    n = s + v + e + p + a + i + j + r + d
    rh = 0
    y0 = s, v, e, p, a, i, j, r, d, rh
    ...
    ret = odeint(derivdecayVNS, y0, t, args=(beta, gamma1, gamma2, alpha, sym, hosp, q, l, n_days,
                                             ...
                                             fatal_hosp, x,  # Change Here for time-point
                                             p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8, p_m9, p_m10, p_m11, p_m12,
                                             delta_p, sigma, phi5, phi6, phi7, phi8, phi9, phi10, phi11, phi12,
...
new_strain6 = (st.sidebar.number_input(
    "New Strain Increased Transmission w.r.t. Old Strain 6 (%)", 0.0, 1000.0, value=40.0, step=5.0,
    format="%f") / 100.0)
...
new_strain7 = (st.sidebar.number_input(
    "New Strain Increased Transmission w.r.t. Old Strain 7 (%)", 0.0, 1000.0, value=40.0, step=5.0,
    format="%f") / 100.0)
...
new_strain8 = (st.sidebar.number_input(
    "New Strain Increased Transmission w.r.t. Old Strain 8 (%)", 0.0, 1000.0, value=100.0, step=5.0,
    format="%f") / 100.0)
...
new_strain9 = (st.sidebar.number_input(
    "New Strain Increased Transmission w.r.t. Old Strain 9 (%)", 0.0, 1000.0, value=100.0, step=5.0,
    format="%f") / 100.0)
...
fracNS10 = (st.sidebar.number_input(
    "Percent of Population with new strain 10 (%)", 0.0, 100.0, value=95.0, step=5.0, format="%f") / 100.0)
new_strain10 = (st.sidebar.number_input(
    "New Strain Increased Transmission w.r.t. Old Strain 10 (%)", 0.0, 1000.0, value=100.0, step=5.0,
    format="%f") / 100.0)

intervention10 = st.sidebar.date_input(
...
p_m11 = (st.sidebar.number_input(
    "Mask-wearing 11", 0.0, 100.0, value=30.0, step=5.0, format="%f") / 100.0)
phi11 = (st.sidebar.number_input(
    "Vaccination Rate 11 (%)", 0.0, 100.0, value=0.1, step=0.5, format="%f") / 100.0)
fracNS11 = (st.sidebar.number_input(
    "Percent of Population with new strain 11 (%)", 0.0, 100.0, value=100.0, step=5.0, format="%f") / 100.0)
new_strain11 = (st.sidebar.number_input(
    "New Strain Increased Transmission w.r.t. Old Strain 11 (%)", 0.0, 1000.0, value=100.0, step=5.0,
    format="%f") / 100.0)
...
decay12 = st.sidebar.number_input(
    "Social distancing 12 - Percent", 0, 100, value=5, step=5, format="%i") / 100.0
p_m12 = (st.sidebar.number_input(
    "Mask-wearing 12", 0.0, 100.0, value=30.0, step=5.0, format="%f") / 100.0)
phi12 = (st.sidebar.number_input(
    "Vaccination Rate 12 (%)", 0.0, 100.0, value=0.1, step=0.5, format="%f") / 100.0)
fracNS12 = (st.sidebar.number_input(
    "Percent of Population with new strain 12 (%)", 0.0, 100.0, value=100.0, step=5.0, format="%f") / 100.0)
new_strain12 = (st.sidebar.number_input(
    "New Strain Increased Transmission w.r.t. Old Strain 12 (%)", 0.0, 1000.0, value=150.0, step=5.0,
    format="%f") / 100.0)
...
q = 1 - (st.sidebar.number_input(
    "Symptomatic Isolation Rate (contact tracing/quarantine when symptomatic)", 0.0, 100.0, value=34.8, step=0.1,
    format="%f") / 100.0)
...
initial_infections = st.sidebar.number_input(
    ...
    value=known_infections, step=10.0, format="%f")
...
beta_j = AAA * (1 / (((1 - asymptomatic) * 1 / gamma2) + (asymptomatic * q / (gamma2 + hosp_rate)) + (
            asymptomatic * hosp_rate * l / ((gamma2 + hosp_rate) * gamma_hosp))))

R0_n = beta_j * (((1 - asymptomatic) * 1 / gamma2) + (asymptomatic * q / (gamma2 + hosp_rate)) + (
            asymptomatic * hosp_rate * l / ((gamma2 + hosp_rate) * gamma_hosp)))
beta_j = 0.51
R0_n = beta_j * (((1 - asymptomatic) * 1 / gamma2) + (asymptomatic * q / (gamma2 + hosp_rate)) + (
            asymptomatic * hosp_rate * l / ((gamma2 + hosp_rate) * gamma_hosp)))

S_p, E_p, P_p, A_p, I_p, J_p, R_p, D_p, RH_p = sim_sepaijrd_decay_ode(S0, E0, P0, A0, I0, J0, R0, D0, beta_j, gamma2,
                                                                      gamma_hosp, alpha, n_days,
                                                                      decay1, decay2, decay3, decay4, decay5, decay6,
                                                                      decay7, decay8, decay9, decay10, decay11, decay12,
                                                                      start_day, int1_delta, int2_delta, int3_delta,
                                                                      int4_delta, int5_delta, int6_delta, int7_delta,
                                                                      int8_delta, int9_delta, int10_delta, int11_delta,
                                                                      fatal_hosp, asymptomatic, hosp_rate, q, l, x,
                                                                      p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7, p_m8,
                                                                      p_m9, p_m10, p_m11, p_m12, delta_p)
...
S_v, V_v, E_v, P_v, A_v, I_v, J_v, R_v, D_v, RH_v = sim_svepaijrd_decay_ode(S0, V0, E0, P0, A0, I0, J0, R0, D0, beta_j,
                                                                            gamma2, gamma_hosp, alpha, n_days,
                                                                            decay1, decay2, decay3, decay4, decay5,
                                                                            decay6, decay7, decay8, decay9, decay10,
                                                                            decay11, decay12, start_day, int1_delta,
                                                                            int2_delta, int3_delta, int4_delta,
                                                                            int5_delta, int6_delta, int7_delta,
                                                                            int8_delta, int9_delta, int10_delta,
                                                                            int11_delta,
                                                                            fatal_hosp, asymptomatic, hosp_rate, q, l,
                                                                            x,
                                                                            p_m1, p_m2, p_m3, p_m4, p_m5, p_m6, p_m7,
                                                                            p_m8, p_m9, p_m10, p_m11, p_m12, delta_p,
                                                                            sigma, phi)
...
S_vNS, V_v, NSE_vNS, P_vNS, A_vNS, I_vNS, J_vNS, R_vNS, D_vNS, RH_vNS = sim_svepaijrdNS_decay_ode(S0, V0, E0, P0, A0,
                                                                                                  I0, J0, R0, D0,
                                                                                                  beta_j, gamma2,
                                                                                                  gamma_hosp, alpha,
                                                                                                  n_days,
...
dispositions_prev["pointprevalencerate"] = (dispositions_prev['Total Prevalent Cases'] / dispositions_prev[
    'Total Cases']) * 100000
...
vertical = pd.DataFrame({'day': [int1_delta, int2_delta, int3_delta, int4_delta, int5_delta, int6_delta, int7_delta,
                                 int8_delta, int9_delta, int10_delta,
                                 int11_delta]})  # add here for additional time-point
...
st.subheader(
    "Projected number of **daily** COVID-19 admissions for Erie County: SEIR - Phase Adjusted R_0 with Case Fatality with Asymptomatic, Pre-Symptomatic, Vaccinations, New Strain transmissibility, and Mask-use")
st.altair_chart(
    # admits_graph_seir
    # +
    # admits_graph
...
st.subheader(
    "Comparison of COVID-19 hospital admissions for Erie County: Model Comparison - Efficacy of Vaccine with New COVID Variant (SVEPAIJRD)")
st.altair_chart(
...
st.altair_chart(
    additional_projections_chart2(dispositions_inc2["incidencerate"], dispositions_prev["pointprevalencerate"]),
    use_container_width=True)
