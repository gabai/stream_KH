...
# Modifiable Model 
def betanew2_0(t,beta,x,p_m1, pm_2 ):
    if start_day<= t <= int1_delta:
        beta_decay=beta*(1-decay1)
    elif int1_delta<=t<int2_delta:
        beta_decay=beta*(1-decay2)
...
def derivdecayP_0(y, t, beta, gamma1, gamma2, alpha, sym, hosp, q, l, n_days, decay1, decay2, decay3, decay4, decay5, decay6, start_day, int1_delta, int2_delta, int3_delta, 
                end_delta, step2_delta, fatal_hosp, x, p_m1, p_m2, delta_p):
    S, E, P,A, I,J, R,D,counter = y
    N=S+E+P+A+I+J+R+D
    dSdt = - betanew2_0(t, beta, x, p_m1, p_m2) * S * (q*I + l*J +P+ A)/N 
    dEdt = betanew2_0(t, beta, x, p_m1, p_m2) * S * (q*I + l*J +P+ A)/N   - alpha * E
...
def sim_sepaijrd_decay_ode_0(
    ...
    t=np.arange(0, n_days, step=1)
    ret = odeint(derivdecayP_0, y0, t, args=(beta, gamma1, gamma2, alpha, sym, hosp,q,l, n_days, decay1, decay2, decay3, decay4, decay5, decay6, start_day, int1_delta,
...
####The SIR model differential equations with ODE solver. Presymptomatic and masks
# Model w/ + 10 facemask
def betanew2_1(t,beta,x,p_m1, pm_2 ):
    if start_day<= t <= int1_delta:
        beta_decay=beta*(1-decay1)
    elif int1_delta<=t<int2_delta:
        beta_decay=beta*(1-decay2)
        ...
        beta_decay=beta*(1-decay4)*(1-(x*p_m1+0.10))**2 
    elif end_delta<=t<=step2_delta:
        beta_decay=beta*(1-decay5)*(1-(x*p_m1+0.10))**2 
    else:
        #beta_decay=beta*(1-0.4)*(1-(x*p_m2))**2    
        beta_decay=beta*(1-decay6)*(1-(x*p_m2+0.10))**2    
    return beta_decay

def derivdecayP_1(y, t, beta, gamma1, gamma2, alpha, sym, hosp, q, l, n_days, decay1, decay2, decay3, decay4, decay5, decay6, start_day, int1_delta, int2_delta, int3_delta, 
                end_delta, step2_delta, fatal_hosp, x, p_m1, p_m2, delta_p):
    S, E, P,A, I,J, R,D,counter = y
    N=S+E+P+A+I+J+R+D
    dSdt = - betanew2_1(t, beta, x, p_m1, p_m2) * S * (q*I + l*J +P+ A)/N 
    dEdt = betanew2_1(t, beta, x, p_m1, p_m2) * S * (q*I + l*J +P+ A)/N   - alpha * E
    dPdt = alpha * E - delta_p * P
    dAdt = delta_p* P *(1-sym)-gamma1*A
    dIdt = sym* delta_p* P - gamma1 * I- hosp*I
    dJdt = hosp * I -gamma2*J
...
def sim_sepaijrd_decay_ode_1(
    ...
    t=np.arange(0, n_days, step=1)
    ret = odeint(derivdecayP_1, y0, t, args=(beta, gamma1, gamma2, alpha, sym, hosp,q,l, n_days, decay1, decay2, decay3, decay4, decay5, decay6, start_day, int1_delta,
                                           int2_delta, int3_delta, end_delta, step2_delta, fatal_hosp, x, p_m1, p_m2, delta_p))
    S_n, E_n,P_n,A_n, I_n,J_n, R_n, D_n ,RH_n= ret.T
...
####The SIR model differential equations with ODE solver. Presymptomatic and masks
# Model w/ - 10 faceask
def betanew2_2(t,beta,x,p_m1, pm_2 ):
    if start_day<= t <= int1_delta:
        beta_decay=beta*(1-decay1)
    elif int1_delta<=t<int2_delta:
        beta_decay=beta*(1-decay2)
...
def derivdecayP_2(y, t, beta, gamma1, gamma2, alpha, sym, hosp, q, l, n_days, decay1, decay2, decay3, decay4, decay5, decay6, start_day, int1_delta, int2_delta, int3_delta, 
                end_delta, step2_delta, fatal_hosp, x, p_m1, p_m2, delta_p):
    S, E, P,A, I,J, R,D,counter = y
    N=S+E+P+A+I+J+R+D
    dSdt = - betanew2_2(t, beta, x, p_m1, p_m2) * S * (q*I + l*J +P+ A)/N 
    dEdt = betanew2_2(t, beta, x, p_m1, p_m2) * S * (q*I + l*J +P+ A)/N   - alpha * E
    dPdt = alpha * E - delta_p * P
    dAdt = delta_p* P *(1-sym)-gamma1*A
    dIdt = sym* delta_p* P - gamma1 * I- hosp*I
    dJdt = hosp * I -gamma2*J
...
def sim_sepaijrd_decay_ode_2(
    ...
    t=np.arange(0, n_days, step=1)
    ret = odeint(derivdecayP_2, y0, t, args=(beta, gamma1, gamma2, alpha, sym, hosp,q,l, n_days, decay1, decay2, decay3, decay4, decay5, decay6, start_day, int1_delta,
                                           int2_delta, int3_delta, end_delta, step2_delta, fatal_hosp, x, p_m1, p_m2, delta_p))
    S_n, E_n,P_n,A_n, I_n,J_n, R_n, D_n ,RH_n= ret.T
...
decay3 = st.sidebar.number_input(
    "Social distancing (% reduction in social contact) from Week 3 to change in SD - After Business Closure%", 0, 100, value=68 ,step=5, format="%i")/100.0
...
decay4 = st.sidebar.number_input(
    "NYS Facemask Mandate", 0, 100, value=45 ,step=5, format="%i")/100.0
...
decay6 = st.sidebar.number_input(
    "Step 2 reduction in social distancing %", 0, 100, value=15 ,step=5, format="%i")/100.0


hosp_rate = (
    st.sidebar.number_input("Hospitalization %", 0.0, 100.0, value=0.2, step=0.50, format="%f")/ 100.0)
...
fatal_hosp = st.sidebar.number_input(
    "Hospital Fatality (%)", 0.0, 100.0, value=4.0 ,step=0.5, format="%f")/100.0
...
asymptomatic = 1-(st.sidebar.number_input(
    "Asymptomatic (%)", 0.0, 100.0, value=38.0 ,step=5.0, format="%f")/100.0)
...
"Percent of people adhering to mask-wearing after April 15,2020", 0.0, 100.0, value=42.0 ,step=5.0, format="%f")/100.0)

p_m2 = (st.sidebar.number_input(
"Percent of people adhering to mask-wearing during Phased transitioning", 0.0, 100.0, value=45.0 ,step=5.0, format="%f")/100.0)
...
if as_date:
    ...
    def erie_icu(projection_admits: pd.DataFrame) -> alt.Chart:
        ...
        projection_admits = projection_admits.rename(columns={"ICU": "Erie County ICU Cases"})
        
        return(
            alt
            .Chart(projection_admits)
            .transform_fold(fold=["Erie County ICU Cases"])
            .mark_line(strokeWidth=3, strokeDash=[2,3], point=True)
            .encode(
                x=alt.X(day_date),
                y=alt.Y("value:Q", title="Census"),
...
S_p, E_p,P_p,A_p, I_p,J_p, R_p, D_p, RH_p=sim_sepaijrd_decay_ode_0(S0, E0, P0,A0,I0,J0, R0, D0, beta_j,gamma2, gamma_hosp, alpha, n_days,
                                                      decay1,decay2,decay3, decay4, decay5, decay6, start_day, int1_delta, int2_delta, int3_delta,
...
S_p, E_p,P_p,A_p, I_p,J_p, R_p, D_p, RH_p=sim_sepaijrd_decay_ode_1(S0, E0, P0,A0,I0,J0, R0, D0, beta_j,gamma2, gamma_hosp, alpha, n_days,
                                                      decay1,decay2,decay3, decay4, decay5, decay6, start_day, int1_delta, int2_delta, int3_delta,
                                                      end_delta, step2_delta, fatal_hosp, asymptomatic, hosp_rate, q, l, x, p_m1, p_m2, delta_p)
...
dispositions_P_ecases_1 = (
            i_hospitalized_P + r_hospitalized_P+ d_hospitalized_P ,
            i_icu_P+r_icu_P+d_icu_P,
            i_ventilated_P+r_ventilated_P +d_ventilated_P)

hospitalized_P_ecases_1, icu_P_1, ventilated_P_1 = (
            i_hospitalized_P,
            i_icu_P,
            i_ventilated_P)
...
S_p, E_p,P_p,A_p, I_p,J_p, R_p, D_p, RH_p=sim_sepaijrd_decay_ode_2(S0, E0, P0,A0,I0,J0, R0, D0, beta_j,gamma2, gamma_hosp, alpha, n_days,
                                                      decay1,decay2,decay3, decay4, decay5, decay6, start_day, int1_delta, int2_delta, int3_delta,
                                                      end_delta, step2_delta, fatal_hosp, asymptomatic, hosp_rate, q, l, x, p_m1, p_m2, delta_p)
...
dispositions_P_ecases_2 = (
            i_hospitalized_P + r_hospitalized_P+ d_hospitalized_P ,
            i_icu_P+r_icu_P+d_icu_P,
            i_ventilated_P+r_ventilated_P +d_ventilated_P)

hospitalized_P_ecases_2, icu_P_2, ventilated_P_2 = (
            i_hospitalized_P,
            i_icu_P,
            i_ventilated_P)
...
#############
# SEPAIJRD Model - Original
# New Cases
projection_admits_P_ecases_0 = build_admissions_df_n(dispositions_P_ecases_0)
## Census Table
census_table_P_ecases_0 = build_census_df(projection_admits_P_ecases_0)

#############
# SEPAIJRD Model - +10
# New Cases
projection_admits_P_ecases_1 = build_admissions_df_n(dispositions_P_ecases_1)
## Census Table
census_table_P_ecases_1 = build_census_df(projection_admits_P_ecases_1)

#############
# SEPAIJRD Model - -10
# New Cases
projection_admits_P_ecases_2 = build_admissions_df_n(dispositions_P_ecases_2)
## Census Table
census_table_P_ecases_2 = build_census_df(projection_admits_P_ecases_2)
...
admits_graph_A= regional_admissions_chart(projection_admits_A_ecases, 
        ...
        as_date=as_date)
# SEPAIJRD Model v0
admits_graph_P_0= regional_admissions_chart(projection_admits_P_ecases_0, 
        plot_projection_days, 
        as_date=as_date)
...
if model_options == "ICU":
    columns_comp_census = {"icu": "Predicted ICU Census"}
    fold_comp_census = ["Predicted ICU Census"]
...
seir_P_ip_ecases_0 = ip_census_chart(census_table_P_ecases_0, plot_projection_days, as_date=as_date)
seir_P_ip_ecases_1 = ip_census_chart(census_table_P_ecases_1, plot_projection_days, as_date=as_date)
seir_P_ip_ecases_2 = ip_census_chart(census_table_P_ecases_2, plot_projection_days, as_date=as_date)
...
st.altair_chart(
    ...
    #+
    alt.layer(seir_P_ip_ecases_0.mark_line())
    + alt.layer(seir_P_ip_ecases_1.mark_line())
    + alt.layer(seir_P_ip_ecases_2.mark_line())
