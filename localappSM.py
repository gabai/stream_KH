...
def sim_seird_decay(
    ...
    decay1:float, decay2:float, decay3: float, decay4: float, end_delta: int, fatal: float, N:float
    ) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Simulate the SIR model forward in time."""
    ...
    for day in range(n_days):
        if start_day<=day<=int1_delta:
            beta_n = (alpha+(2 ** (1 / 1.61) - 1))*((2 ** (1 / 1.61) - 1) + (gamma)) / (alpha*N)
            beta_decay=beta_n*(1-decay1)
        elif int1_delta<=day<=int2_delta:
            beta_n = (alpha+(2 ** (1 / 2.65) - 1))*((2 ** (1 / 2.65) - 1)+ (gamma)) / (alpha*N)
            beta_decay=beta_n*(1-decay2)
        elif int2_delta<=day<=end_delta:
            beta_n = (alpha+(2 ** (1 / 5.32) - 1))*((2 ** (1 / 5.32) - 1)+ (gamma)) / (alpha*N)
            beta_decay=beta_n*(1-decay3)
        else:
            beta_n = (alpha+(2 ** (1 / 9.70) - 1))*((2 ** (1 / 9.70) - 1)+ (gamma)) / (alpha*N)
            beta_decay=beta_n*(1-decay4)
...
s_D, e_D, i_D, r_D, d_D = sim_seird_decay(S-2.0, 1.0, 1.0 , 0.0, 0.0, beta4, gamma2,alpha, n_days, decay1, decay2, decay3, decay4, end_delta, fatal, S-2.0)
